...
export class EnhancePromptCommand {
  ...
  async execute(): Promise<void> {
    try {
      ...
      const context = TextProcessor.createEnhancementContext(editor);
      if (!context) {
      ...
      const outputAction = await QuickPickManager.showOutputActionSelector();
      if (!outputAction) {
  ...
  private async showSettingsQuickPick(): Promise<void> {
    ...
    switch (selected.label.split(' ')[1]) {
      case 'Model':
        const newModel = await QuickPickManager.showModelSelector(settings.model);
        if (newModel) {
